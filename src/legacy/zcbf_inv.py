...
class ClfIrisController(object):
    def __init__(self):
        ...
        "[[-8.96,-15.52,8.00, 0,0,0, 0,0,0, 1.00]]")
    ...
    def loop(self,_):
        ...
        p_ros = self.last.pose.pose.position
        ...
        if self.use_gz:
            ...
            omega_body_frame  = np.dot(R_mat.T, omega_world_frame)
        else:
            v_in_body_frame = np.array([v_raw_linear.x, v_raw_linear.y, v_raw_linear.z])
            v_w_world    = np.dot(R_mat, v_in_body_frame)
        ...
        if self.state in (State.TAKEOFF,State.HOVER):
            ...
            err_p_vec = p_vec - tgt
            err_z  = abs(err_p_vec[2])
            err_v = np.linalg.norm(v_w_world - vd)
        ...
        e_th = np.array([phi-phi_d,
                         ...
                         -math.pi])
        e_w  = (omega_body_frame - np.array([0.0,0.0,rd]))

        U2_nom = (self.Ix*(-g3*e_th[0] - g4*e_w[0])
              - omega_body_frame[1]*omega_body_frame[2]*(self.Iy-self.Iz))
        U3_nom = (self.Iy*(-g3*e_th[1] - g4*e_w[1])
              - omega_body_frame[0]*omega_body_frame[2]*(self.Iz-self.Ix))
        U4_nom = (self.Iz*(-g3*e_th[2] - g4*e_w[2])
              - omega_body_frame[0]*omega_body_frame[1]*(self.Ix-self.Iy))
        U4_nom = np.clip(U4_nom, -0.75*U1_nom*self.km/self.kf , 0.75*U1_nom*self.km/self.kf)
        ...
        if self.state == State.TRAJ and self.obs.size > 0:
            ...
            for obs_row in self.obs:
                ...
                R_Omxe3 = R_mat.dot(np.cross(omega_body_frame, e3_body))
                ...
                Omega_cross_e3 = np.cross(omega_body_frame, e3_body)
                Omega_cross_Omega_cross_e3 = np.cross(omega_body_frame, Omega_cross_e3)
                R_Omega_cross_Omega_cross_e3 = np.dot(R_mat, Omega_cross_Omega_cross_e3)
                J_Omega = np.dot(self.J_mat, omega_body_frame)
                Omega_cross_JOmega = np.cross(omega_body_frame, J_Omega)
