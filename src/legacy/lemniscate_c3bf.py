...
class ClfIrisController(object):
    def __init__(self):
        ...
        self.omega_traj = pget("trajectory_omega", 0.1) # Base angular frequency
    ...
    def traj_ref(self, t):
        w = self.omega_traj
        a = self.traj_a
        b = a / 2.0 # Standard figure-eight proportion

        # Lissajous Position (flat in Z)
        theta = w * t
        x_rel = a * math.cos(theta)
        y_rel = b * math.sin(2.0 * theta)
        z_rel = 0.0 # Flat trajectory at takeoff height

        # Position vector relative to takeoff Z height
        pos = np.array([x_rel, y_rel, z_rel])

        # Apply offsets calculated during HOVER->TRAJ transition
        if self.xy_offset is not None:
            pos[0:2] += self.xy_offset
        # Z offset likely corresponds to takeoff height, add it
        if self.z_offset is not None:
             pos[2] += self.z_offset
        else: # Fallback if z_offset wasn't set (e.g., skipping hover)
             pos[2] += self.z_to


        # Velocity (derivative of pos w.r.t. time)
        dx_dt = -a * w * math.sin(theta)
        dy_dt = b * 2.0 * w * math.cos(2.0 * theta)
        dz_dt = 0.0
        vel = np.array([dx_dt, dy_dt, dz_dt])

        # Acceleration (derivative of vel w.r.t. time)
        d2x_dt2 = -a * w**2 * math.cos(theta)
        d2y_dt2 = -b * 4.0 * w**2 * math.sin(2.0 * theta)
        d2z_dt2 = 0.0
        acc = np.array([d2x_dt2, d2y_dt2, d2z_dt2])
    ...
    def build_c3bf_constraints(self, p_world, v_world):
        ...
        for i in range(self.obs.shape[0]):
            ...
            d = rt(d2)
            buffer = 0.30
            # Adjusted skipping condition margin slightly
            if d <= r_s + EPS- buffer:
            ...
            if self.state == State.HOVER:
                if (err_pos < hover_pos_thresh and err_v < hover_vel_thresh):
                    if self.hover_ok_t is None:
                        ...
                        self.t0_traj = now
                        # Get reference start point (relative to origin)
                        ref_pos_start_rel, _, _, _, _ = self.traj_ref(0.0) # This gives [a, 0, 0]
                        # Calculate offsets needed to start traj from current hover pos
                        self.xy_offset = p_world[0:2] - ref_pos_start_rel[0:2]
                        self.z_offset  = p_world[2] - ref_pos_start_rel[2] # z_offset is simply the hover height
                else:
                    self.hover_ok_t = None
