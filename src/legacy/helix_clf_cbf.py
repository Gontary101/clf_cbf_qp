    ...
    def loop(self,_):
        ...
        if self.state == State.TRAJ and self.obs.size > 0:
            ...
            for ox,oy,oz,r in self.obs:
                ...
                Bd  = 2 * pr.dot(v_rel)
                psi_cbf = 2*v_rel.dot(v_rel) + self.k1*Bd + self.k0*Br
                G_list.append(-2*pr)
                h_list.append(psi_cbf)
                ...
                try:
                    ...
                    if sol['status'] == 'optimal':
                        ...
                        self.cbf_pub.publish(Float64MultiArray(data=list(slack)))
                         ...
                         ad = ad_nom
                         self.cbf_pub.publish(Float64MultiArray(data=[0.0]*len(h_list)))
                except ValueError:
                    rospy.logwarn_throttle(1.0, "CBF-QP infeasible, using nominal accel")
                    ad = ad_nom
                    self.cbf_pub.publish(Float64MultiArray(data=[0.0]*len(h_list)))
            else:
                 ad = ad_nom
