...
class GazeboObstacleProcessor(object):
    def __init__(self, ns, all_drone_namespaces, pget_func):
        ...
        # Only use ellipsoid approximations; remove sphere-based fallback
        self.gz_ellip_obs = np.empty((0,22), dtype=float)  # [x,y,z,vx,vy,vz,ax,ay,az,a,b,c,n] + rot_flat
    ...
    def process_model_states_msg(self, msg):
        """
        Populate only ellipsoid approximations for all non-ego models.
        """
        ellips = []
        now = rospy.Time.now()
        for i, name in enumerate(msg.name):
            if name == self.ns:
                continue
            # Generate single superellipsoid for model
            ellips.append(
                self._ellipsoid_for_model(name, msg.pose[i], msg.twist[i])
            )
        # Store ellipsoid list
        self.gz_ellip_obs = np.array(ellips, dtype=float) if ellips else np.empty((0,22), dtype=float)

    def get_combined_obstacles(self, current_drone_p_vec_numpy):
        """
        Return only ellipsoid obstacles (static + dynamic), filtered by range.
        """
        # Combine static ellipsoids with any dynamic ellipsoids (if used)
        combined = self.gz_ellip_obs.copy()
        if combined.size == 0:
            return combined
        # Ground clearance filter
        z_ground_tol = float(self.pget_func("z_ground_tol", 0.0))
        combined = combined[combined[:,2] > z_ground_tol]
        if combined.size == 0:
            return combined
        # Range filter
