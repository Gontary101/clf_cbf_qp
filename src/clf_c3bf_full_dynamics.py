...
class ClfIrisController(object):
    ...
    def build_c3bf_constraints(self, p_world, v_world):
        ...
        for i in range(self.obs.shape[0]):
            ...
            safe_div = 0.0 if s < EPS else s / (rho + EPS)
            Lfh  = s2 + safe_div * p_dot_v          # as before
            Lgh  = p_rel + v_rel * (rho / (s + EPS))         # as before
            Lf_bar = Lfh - self.g * Lgh[2]                   # include gravity
            Lg_bar = Lgh / self.m                            

            G_row   = -Lg_bar
            b_scalar = Lf_bar + self.gamma * h_i
            Gs.append(G_row)
            hs.append(b_scalar)
    ...
    def loop(self, event):
        ...
        v_world = dot(self.R(phi, th, psi), [v3.x, v3.y, v3.z])
        ...
        if self.state == State.TRAJ and self.obs.size > 0:
            G_cbf, h_cbf, h_debug_data = self.build_c3bf_constraints(p_world, v_world)

            if G_cbf is not None:
                G_extra, h_extra = [], []
                G_extra.append([0, 0,  1])
                h_extra.append(self.Fz_max)
                
                rows = [np.asarray(G_cbf, dtype=float)]
                rows.append(np.array(G_extra, dtype=float))
                G = matrix(np.vstack(rows))
                h = matrix(np.hstack([np.asarray(h_cbf, dtype=float).ravel(),
                                      np.array(h_extra,  dtype=float)]))

            else:
                G_extra, h_extra = [], []
                G_extra.append([0, 0,  1])
                h_extra.append(self.Fz_max)
                G = matrix(np.array(G_extra, dtype=float))
                h = matrix(np.array(h_extra,  dtype=float))
            F_nom = self.m * (ad_nom + np.array([0.0, 0.0, self.g]))
            P = matrix(np.eye(3))
            q = matrix(-F_nom)
            ...
            if G is not None:
                try:
                    ...
                    if sol['status'] == 'optimal':
                        ...
                        self.last_F_safe = F_safe.copy()
                        acc_safe = F_safe / self.m - np.array([0.0, 0.0, self.g])
                        slack = h - G * sol['x']
                        self.cbf_slack_pub.publish(Float64MultiArray(data=list(slack)))
                         ...
                         F_safe = getattr(self, "last_F_safe", F_nom)
                         acc_safe = F_safe / self.m - np.array([0.0, 0.0, self.g])
                         self.cbf_slack_pub.publish(Float64MultiArray(data=[]))
                except ValueError as e:
                    rospy.logwarn_throttle(1.0, "C3BF-QP ValueError (likely infeasible): %s. Re-using previous safe force.", e)
                    F_safe = getattr(self, "last_F_safe", F_nom)
                    acc_safe = F_safe / self.m - np.array([0.0, 0.0, self.g])
                    self.cbf_slack_pub.publish(Float64MultiArray(data=[]))
                except ArithmeticError as e:
                     rospy.logwarn_throttle(1.0, "C3BF-QP ArithmeticError: %s. Re-using previous safe force.", e)
                     F_safe = getattr(self, "last_F_safe", F_nom)
                     acc_safe = F_safe / self.m - np.array([0.0, 0.0, self.g])
                     self.cbf_slack_pub.publish(Float64MultiArray(data=[]))
                except Exception as e:
    ...
    def shutdown(self):
        ...
        count = 0
        # Try sending stop commands for a short duration
        while count < 10 and not rospy.is_shutdown():
             ...
             try:
                  ...
                  pass # Ignore if time jumps during shutdown
...
if __name__ == "__main__":
    ...
    try:
        ...
        traceback.print_exc() # Print full traceback for debugging
         ...
         # Ensure shutdown sequence runs even if error occurs during init/spin
