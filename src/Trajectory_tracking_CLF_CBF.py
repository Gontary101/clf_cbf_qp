...
class ClfIrisController(object):
    def __init__(self):
        ...
        self.max_tilt    = math.radians(pget("max_tilt_angle_deg",15.0))

        # trajectory + take‑off targets ---------------------------------------------
        self.a, self.b   = pget("ellipse_a",32.0), pget("ellipse_b",22.0)
        self.z_traj      = pget("trajectory_z",5.0)
        ...
        #  ----- subscribe to SAFE virtual inputs produced by the CBF ---------------
        rospy.Subscriber("~control/virtual_inputs_safe", Point,
                         self.cb_safe, queue_size=1)
        self.u_safe = None                       # will hold np.array([Uex,Uey])
    ...
    def cb_safe(self,msg):
        """Store the last CBF-filtered virtual inputs (thread-safe)."""
        self.u_safe = np.array([msg.x, msg.y])
    ...
    def loop(self,_):
        ...
        # ------------------ current pose / twist (world frame) ----------------------
        ...
        # ------------------ choose reference for this control mode -----------------
        if self.state in (State.TAKEOFF,State.HOVER):
            ...
            if self.state==State.TAKEOFF and \
               err_pos< pget("hover_pos_threshold",.15) and err_v<pget("hover_vel_threshold",.1):
        ...
        if U1<1e-6:
            ...
            Uex_nom=(self.m/U1)*(-g1*ex1[0]+ad[0]-g2*ex2[0])
            Uey_nom=(self.m/U1)*(-g1*ex1[1]+ad[1]-g2*ex2[1])

        # ---------- *apply CBF filter output if available --------------------
        if self.u_safe is not None:
            Uex,Uey = self.u_safe            # ← safe virtual inputs
        else:
            Uex,Uey = Uex_nom,Uey_nom        # ← nominal inputs
        ...
        # ------------------ inner‑loop: attitude / body rates -----------------------
