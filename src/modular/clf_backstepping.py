...
class CLFBackstepping(object):
    ...
    def __init__(self, model):
        self.model = model   # instance of dynamics_utils.DroneModel

    # ----------------------------------------------------------------------
    def compute(self, state, ref, gains):
        """
        Compute nominal virtual inputs U = [F, Mx, My, Mz].

        state  - dict  { p_vec, v_vec, phi, th, psi, omega_body, R_mat }
        ref    - dict  { tgt, vd, ad, yd, rd }
        gains  - tuple/list (g1, g2, g3, g4)

        Returns a dict with U_nom and many useful intermediates.
        """
