...
class ZCBFInvController(object):
    ...
    def _parse_obstacles(self):
        default_obs_str = "[]" # Default to no obstacles if not specified or parsing fails
        # Match old code's default if needed, but launch file provides actual obstacles
        # default_obs = [[-8.96, -15.52, 8.00, 0, 0, 0, 0, 0, 0, 1.00]] 
        # default_obs_str = str(default_obs)
